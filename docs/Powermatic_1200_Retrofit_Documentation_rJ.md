...
>   and rates GS20 drives for circuits up to 100 kA RMS symmetrical. For this personal-shop retrofit, the
>   SCCR worksheet is a conservative due-diligence check and field record, not a product-certification gate.
...
>   hold-to-run jogs at 30 Hz** (preset-LOW forced), gated safe/no-fault. **Exit** on **any STOP press**,
>   **60 s idle**, **any fault**, or **E-stop**. Annunciated as **blinking green = JOG ARMED** (see indicators).
...
>
> **This is a personal-shop design package, not a listed control product or a substitute for
> on-machine verification.** The intent is to build close to NFPA 79 / UL 508A practice for shop
> safety, while recognizing that this one-off retrofit is not UL certified. If this design is ever
> sold as a product, pursue the appropriate certification/listing at that time. Field-verify every
...
3. **Keep the original controls and the "chunk."** Drum switch, pendant, and limits are reused as
   24 V inputs. The vintage right-hand Furnas contactor is retained only for the audible chunk; the
   real line/safety contactor is the modern Schneider `LC1D18BD`.
...
The selector is an operating-mode request, not a run command. Changing it while stopped is normal.
Changing it during motion is treated as a soft stop/abort: DRILL latches drop when `X001` is lost,
and TAP states reset when `X002` is lost. Selecting TAP by itself never starts a cycle; TAP can only
start from IDLE on a fresh FWD press with the feed-lever-OFF prox true, STOP clear, drum LOW/HIGH,
safety OK, no immediate trip, no jog mode, no hard fault, and no DB over-temp.
...
- **In jog mode**, the pendant **FWD and REV buttons are momentary hold-to-run jogs** — the spindle turns
  ...
  direction, gated **safe (`X011`) · drum LOW or HIGH · STOP clear · no fault · no over-temp**. Normal
  latched runs and TAP starts are inhibited while armed.
- **Drum OFF does not prevent entering or staying in jog mode.** This is intentional: the operator may
  discover during setup that a different speed band is needed. With the drum OFF, jog remains armed but
  cannot move the spindle; selecting LOW or HIGH restores jog motion without leaving and re-entering jog.
- **Exit** on any of: **any STOP press**, **60 s of jog inactivity**, **any latched fault**, or an
  **E-stop / safety trip**. STOP always stops motion and exits jog; there is no long-STOP-stays-armed mode.
...
| `X004` | STOP | Pendant STOP button | **Wired N.C.**, 1 = *not* pressed (fail-safe); any STOP press exits jog | Both |
...
| `C50` | FWD-press consumed flag (long-hold-entered-jog **or** press-that-stopped-REV → suppress release-latch and entry-hold jog motion) |
...
- **Note F — `C50` is a shared "this FWD press is spent" flag.** It is set either when a 5 s hold enters jog
  ...
  sees `C50=1` and does not latch a forward run. RUNG 24 also requires `¬C50`, so the 5 s entry hold cannot
  start the spindle while the button is still held. `C50` clears on FWD release (RUNG 8), after which a fresh
  FWD or REV press can command jog motion.
...
- **Note I — jog exits on STOP.** STOP is not overloaded with a long-hold distinction in jog mode. Any STOP press
  resets `C60`, stops motion, and returns the pendant to normal DRILL/TAP behavior.
...
- [ ] **SCCR worksheet:** retain the `LC1D18BD` 100 kA / 40 A-maximum-fuse table and the GS20 pages specifying `TJN35` for `GS23-22P0` with 100 kA withstand. Confirm available fault current for the personal-shop field record.
...
- [ ] Hold FWD ≥5 s to arm jog, continue holding FWD after the 5 s mark → spindle does **not** move; release FWD,
      then a fresh FWD/REV press jogs (verifies `¬C50` in RUNG 24).
- [ ] With drum OFF: hold FWD ≥5 s → jog arms (green blink); FWD/REV presses do nothing; set drum to LOW or HIGH
      while still armed → fresh FWD/REV presses now jog. Return drum to OFF mid-jog → motion stops but jog remains armed.
- [ ] Exit on: any STOP press; 60 s idle; any fault; E-stop.
...
| `Powermatic_1200_SCCR_Worksheet_rJ.md` | SCCR component worksheet, verified selected-device conditions, source assumption, and personal-shop field record |
...
PDF helper scripts are tracked in the repository under `scripts/`.
